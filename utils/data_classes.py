...
class KITTISingleObject:
    ...
    @property
    def projected_flex_kpts(self) -> np.ndarray:
        ...
        h_offset = (0.5 * self.h)
        top_center = (self.loc + np.array([0, -1 * h_offset, 0])).reshape(3, 1)
        bottom_center = (self.loc + np.array([0, h_offset, 0])).reshape(3, 1)
        corners_3d = np.concatenate([corners_3d, top_center], axis=1)
        corners_3d = np.concatenate([corners_3d, bottom_center], axis=1)

        original_corners_3d = corners_3d.copy()
        
        in_front = np.argwhere(corners_3d[2, :] > 0).flatten()
        valid_corners_mask = np.zeros((10, 1))
        valid_corners_mask[in_front, :] = 1
        
        
        all_corner_coords = view_points(original_corners_3d, self.calib.P2, normalize=True).T[:, :2]
        projected_pts = np.hstack([all_corner_coords, valid_corners_mask])
