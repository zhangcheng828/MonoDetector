...
def move_ddp_data_device(data_dict: Dict[str, Any], 
                     device: str = None) -> Dict[str, Any]:
    ...
    for k, v in data_dict.items():
        if isinstance(v, torch.Tensor):
            data_dict[k] = data_dict[k].cuda(device, non_blocking=True)

    if 'label' in data_dict.keys():
        label = data_dict['label']
        for k in label.keys():
            label[k] = label[k].cuda(device, non_blocking=True)
        data_dict['label'] = label
...
def accumulate_losses(loss_dict: Dict[str, torch.Tensor], loss_weights) -> torch.Tensor:
    total_loss = torch.zeros(1).cuda()
    for key in loss_weights.keys():
        total_loss += loss_weights[key].detach()*loss_dict[key]
    return total_loss
