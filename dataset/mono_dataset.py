...
DEFAULT_FILTER_CONFIG = {
    ...
    'max_truncation': 0.5,
    'max_occlusion': 2,
    'kpts':9, # 10 for monoflex and 9 for monocon
}
...
class MonoDataset(BaseKITTIMono3DDataset):
    def __init__(self, 
                 base_root: str, 
                 split: str,
                 max_objs: int = 30,
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # coord_range = np.array([[0, 0], [input_hw[1], input_hw[0]]]).astype(np.float32)
        for obj_idx, raw_label in enumerate(raw_labels):
            ...
            if self.kpts == 9:
                ...
                keypoints = raw_label.projected_flex_kpts
            ...
            if trunc > 0:
                new_labels['trunc_mask'][obj_idx] = True
            ...
            new_labels['calib_f'][obj_idx] = calib.fu
    ...
    def _create_empty_labels(self) -> Dict[str, np.ndarray]:
        annot_dict = {
            ...
            'calib_f': np.zeros(self.max_objs, dtype=np.float32),
            'gt_kpts_2d': np.zeros((self.max_objs, 2 * self.kpts), dtype=np.float32),
            'gt_kpts_valid_mask': np.zeros((self.max_objs, self.kpts), dtype=np.uint8),
            'mask': np.zeros((self.max_objs,), dtype=np.bool_),
            'trunc_mask': np.zeros((self.max_objs,), dtype=np.bool_)}
        return annot_dict
