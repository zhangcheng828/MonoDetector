...
from model import DLA, DLAUp, SmokeDenseHeads, GUPNetDenseHeads, MonoConDenseHeads, MonoFlexDenseHeads, IDMSDenseHeads
...
class BaseMonoDetector(nn.Module):
    def __init__(self,
                 num_dla_layers: int = 34,
                 pretrained_backbone: bool = True,
                 dense_heads: str = 'SmokeDetector',
        ...
        if dense_heads == 'SmokeDetector':
            self.head = SmokeDenseHeads(in_ch=head_in_ch, test_config=test_config, **head_config)
        elif dense_heads == 'GUPNetDetector':
            self.head = GUPNetDenseHeads(in_ch=head_in_ch, test_config=test_config, **head_config)
        elif dense_heads == 'MonoConDetector':
            self.head = MonoConDenseHeads(in_ch=head_in_ch, test_config=test_config, **head_config)
        elif dense_heads == 'MonoFlexDetector':
            self.head = MonoFlexDenseHeads(in_ch=head_in_ch, test_config=test_config, **head_config)
        elif dense_heads == 'IDMSDetector':
            self.head = IDMSDenseHeads(in_ch=head_in_ch, test_config=test_config, **head_config)
    ...
    def _extract_feat_from_data_dict(self, data_dict: Dict[str, Any]) -> torch.Tensor:
        ...
        return self.neck(backbone_feature)[0]
