...
from utils.idms_target_generator import IDMSTargetGenerator
from utils.kitti_convert_utils import convert_to_kitti_3d

# Constants
EPS = 1e-12

class IDMSDenseHeads(BaseDenseHeads):
    def __init__(self,
                 in_ch: int = 64,
                 feat_ch: int = 64,
                 num_kpts: int = 9,
        ...
        # Target Generator
        self.target_generator = IDMSTargetGenerator(
        ...
        self.crit_offset3d = L1Loss(loss_weight=1.0)
        self.crit_ins_depth = L1Loss(loss_weight=1.0)
    ...
    def init_weights(self, prior_prob: float = 0.1) -> None:
        ...
        for head in [self.offset2d_head, self.ins_depth_head, self.wh_head, self.offset3d_head,
    ...
    def _get_predictions(self, feat: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # (2) Offset
        offset3d_pred = self.offset3d_head(feat)
        offset2d_pred = self.offset2d_head(feat)
        wh_pred = self.wh_head(feat)
        # instance depth
        ins_depth_pred = self.ins_depth_head(feat)
        ...
        return {
            ...
            'offset3d_pred': offset3d_pred,
            'wh_pred': wh_pred,
            'ins_depth_pred': ins_depth_pred,
    ...
    def decode_heatmap(self, 
        ...
        offset = transpose_and_gather_feat(pred_dict['offset3d_pred'], indices)       # (B, K, 2)
    ...
    def _get_losses(self, 
        ...
        # Offset Loss
        offset3d_pred = extract_input(pred_dict['offset3d_pred'], indices, mask_target)
        offset3d_target = extract_target(target_dict['offset3d_target'], mask_target)
        offset3d_loss = self.crit_offset3d(offset3d_pred, offset3d_target)

        offset2d_pred = extract_input(pred_dict['offset2d_pred'], indices, mask_target)
        offset2d_target = extract_target(target_dict['offset2d_target'], mask_target)
        offset2d_loss = self.crit_offset2d(offset2d_pred, offset2d_target)
        ...
        #
        # Heatmap Losses and Instance depth loss
        #
        
        ct_heat_loss = self.crit_center_heatmap(pred_dict['center_heatmap_pred'], target_dict['center_heatmap_target'])    
        ins_depth_loss = self.crit_ins_depth(pred_dict['ins_depth_pred'] * target_dict['mask_ins_depth'], 
                                            target_dict['ins_depth_target'])    
        #
        # Alpha Losses
        #
        ...
        return {
            ...
            'loss_offset2d': offset2d_loss,
            'loss_offset3d': offset3d_loss,
            'loss_wh': wh_loss,
            'loss_ins_depth': ins_depth_loss,
            'loss_dim': dim_loss,
